...
class DCGAN(object):
        ...
        if self.dataset_name == 'mnist':
            ...
            self.data = [
                fpath for dpath in os.walk(
                    os.path.join(
                        "./data",
                        self.dataset_name)) for fpath in glob(
                    os.path.join(
                        dpath[0],
                        self.input_fname_pattern))]
            imreadImg = imread(self.data[0])
            if len(
                    imreadImg.shape) >= 3:  # check if image is a non-grayscale image by checking channel number
    ...
    def build_model(self):
        ...
        if self.y_dim:
            with tf.variable_scope("generator") as scope:
                self.G = self.generator(self.z, self.y, train=True)
                scope.reuse_variables()
                self.sampler = self.generator(self.z, self.y)

            with tf.variable_scope("discriminator") as scope:
                self.D, self.D_logits = self.discriminator(inputs, self.y)
                scope.reuse_variables()
                self.D_, self.D_logits_ = self.discriminator(self.G, self.y)
        else:
            with tf.variable_scope("generator") as scope:
                self.G = self.generator(self.z, train=True)
                scope.reuse_variables()
    ...
    def generator(self, z, y=None, train=False):
        if not self.y_dim:
