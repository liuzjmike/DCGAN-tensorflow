...
def visualize(sess, dcgan, config, option):
    ...
    def get_filepath(fname):
        return os.path.join(config.sample_dir, dcgan.model_dir, fname)
    if option == 0:
        ...
        save_images(
            samples, [
                image_frame_dim, image_frame_dim], get_filepath(
                'test_%s.png' %
                strftime(
                    "%Y%m%d%H%M%S", gmtime())))
    elif option == 1:
        ...
        for idx in [random.randint(0, 99) for _ in xrange(100)]:
            ...
            try:
                make_gif(samples, get_filepath('test_gif_%s.gif' % (idx)))
            except BaseException:
                save_images(
                    samples, [
                        image_frame_dim, image_frame_dim], get_filepath(
        ...
        for idx in xrange(100):
            ...
            image_set.append(
                sess.run(
                    ...
                    feed_dict={
                        dcgan.z: z_sample}))
            make_gif(image_set[-1], get_filepath('test_gif_%s.gif' % (idx)))

        new_image_set = [merge(np.array([images[idx] for images in image_set]), [
